...
const columnRefs = (data, target) => {
  ...
  Object.values(target.keys).filter(k => !k.isAssociation && !k.virtual).forEach(k => columns.add(k.name))
...
module.exports = class Differ {
  ...
  async _diffUpdate(req, providedData) {
    // prepare before data
    let dbState
    if (cds.db) { // does the code below ever work without partialPersistentState?
