...
class PGFunctionBuilder extends FunctionBuilder {
  ...
  _handleCurrentTimestamp () {
    let currentDate = new Date().toISOString()
    currentDate = currentDate.replace('T', ' ').replace('Z', '').replace('+0000', '').replace('+00:00', '')
    this._outputObj.sql.push(`'${currentDate}'`)
  }
