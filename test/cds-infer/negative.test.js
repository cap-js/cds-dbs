...
const { expect } = cds.test.in(__dirname+'/../cqn4sql/bookshop')

describe('negative', () => {
  let model
  beforeAll(async () => {
    model = cds.model = await await cds.load(__dirname + '/../cqn4sql/bookshop/db/schema').then(cds.linked)
  })

  describe('filters', () => {
    it('filter must not be provided along a structure in column', () => {
      expect(() =>
        cds.inferred(CQL`SELECT from bookshop.Books { ID, dedication[text='foo'].sub.foo }`, model)
      )
  ...
  describe('reference not resolvable', () => {
    ...
    it('element can\'t be found', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(/"boz" not found in the elements of "bookshop.Books"/) // revisit: or Foo?
    })

    it('deeply nested element is not found', () => {
      let query = CQL`SELECT from bookshop.Books as Foo { Foo.dedication.sub.boz }`
      expect(() => cds.inferred(query)).to.throw(/"boz" not found in "bookshop.Books:dedication.sub"/) // revisit: Foo:dedication.sub ?
    })
    ...
    it('element can\'t be found in elements of subquery', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(/"ID" not found in the elements of "Foo"/)
    })
    ...
    it('intermediate step in path expression is not found', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(/"notExisting" not found in "bookshop.Books"/) // or Foo:dedication.sub ?
    })
    ...
    it('table alias shadows element of query source', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(/"text" not found in "bookshop.Books"/)
    })

    it('first path step of from is not resolvable without absolute name', () => {
      let query = CQL`SELECT from Books`
      expect(() => cds.inferred(query)).to.throw(/"Books" not found in the definitions of your model/)
    })
    ...
    it('user defined table alias overwrites implicit table alias', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(/"Books" not found in the elements of "bookshop.Books"/)
    })

    it('$self reference is not found in the queries elements', () => {
      let query = CQL`SELECT from bookshop.Books { ID, $self.boo }`
      expect(() => cds.inferred(query)).to.throw(/"boo" not found in the columns list of query/) // revisit: error message
    })
    ...
    it('filter path is not resolvable in column', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(/"title" not found in "addressee"/) // revisit: better error location "bookshop.Books:dedication.addressee"
    })

    it('filter path is not resolvable in where', () => {
      let query = CQL`SELECT from bookshop.Books where exists author[title = 'Harry Potter']`
      expect(() => cds.inferred(query)).to.throw(/"title" not found in "author"/) // revisit: better error location ""bookshop.Books:author"
    })
    ...
    it('$self reference is not found in the query elements -> cds.infer hints alternatives', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(
    ...
    it('lone $self ref is treated as regular element', () => {
      ...
      expect(() => cds.inferred(query)).to.throw(/"\$self" not found in the elements of "bookshop.Books"/)
    })

    it('lone query alias ref is treated as regular element', () => {
      let query = CQL`SELECT from bookshop.Books as Foo { Foo }`
      expect(() => cds.inferred(query)).to.throw(/"Foo" not found in the elements of "bookshop.Books"/) // or Foo?
    })

    it('scoped query does not end in queryable artifact', () => {
      let query = CQL`SELECT from bookshop.Books:name { * }` // name does not exist
      expect(() => cds.inferred(query)).to.throw(/No association "name" in entity "bookshop.Books"/)
      let fromEndsWithScalar = CQL`SELECT from bookshop.Books:title { * }`
      expect(() => cds.inferred(fromEndsWithScalar)).to.throw(
  ...
  describe('expressions', () => {
    it('expression needs alias', () => {
      ...
      expect(() => cds.inferred(expressionWithoutAlias)).to.throw(/Expecting expression to have an alias name/)
      expect(() => cds.inferred(subqueryExpressionWithoutAlias)).to.throw(/Expecting expression to have an alias name/)
    })
    it('no cast on structure', () => {
      ...
      expect(() => cds.inferred(castOnStruct)).to.throw(/Structured elements can't be cast to a different type/)
      expect(() => cds.inferred(castFuncOnStruct)).to.throw(/Structured elements can't be cast to a different type/)
    })
  ...
  describe('ambiguites', () => {
    ...
    it('anonymous functions are inferred by their func property name, ambiguities are rejected', () => {
      ...
      expect(() => cds.inferred(ambiguousFunctions)).to.throw(/Duplicate definition of element “sum”/)
    })
    ...
    it('multiple subqueries/xprs have same alias', () => {
      ...
      expect(() => cds.inferred(ambiguousFunctions)).to.throw(/Duplicate definition of element “foo”/)
    })
    ...
    it('value has same (implicit) alias as other column', () => {
      ...
      expect(() => cds.inferred(ambiguousFunctions)).to.throw(/Duplicate definition of element “false”/)
    })
    ...
    describe('with multiple query sources', () => {
      ...
      it('element reference is ambiguous', () => {
        ...
        expect(() => cds.inferred(query)).to.throw(/ambiguous reference to "ID", write "Books.ID", "Authors.ID" instead/)
      })

      it('table alias is ambiguous', () => {
        let query = CQL`SELECT from bookshop.Books, bookshop.Books { * }`
        expect(() => cds.inferred(query)).to.throw(/Duplicate alias "Books"/)
      })
      ...
      it('table alias via association is ambiguous', () => {
        ...
        expect(() => cds.inferred(query)).to.throw(/Duplicate alias "author"/)
      })

      it('wildcard (no projection) is ambiguous', () => {
        let query = CQL`SELECT from bookshop.Books, bookshop.Foo`
        expect(() => cds.inferred(query)).to.throw(/select "ID" explicitly with "Books.ID", "Foo.ID"/)
      })
