...
describe('nested projections', () => {
  describe('expand', () => {
    ...
    describe('structs', () => {
      it('simple element access', () => {
        ...
        let inferred = cds.inferred(query)
  ...
  describe('inline', () => {
    ...
    it('prefix notation equivalent to structured access', () => {
      ...
      let inferredInline = cds.inferred(queryInlineNotation)
      let inferredAbsolute = cds.inferred(queryStructuredAccess)
    ...
    it('mixed with expand', () => {
      ...
      let inferredInline = cds.inferred(queryInlineNotation)
      let inferredAbsolute = cds.inferred(variantWithoutInline)
      let { Employee } = model.entities
      expect(inferredInline.elements)
        .to.deep.equal(inferredAbsolute.elements)
        .to.deep.equal({
    ...
    it ('deep expand in inline', () => { // revisit: naming
      ...
      let inferredInline = cds.inferred(queryInlineNotation)
      let inferredStruct = cds.inferred(variantWithoutInline)
      let { Employee } = model.entities
      expect(inferredInline.elements)
    ...
    it('wildcard inline toplevel', () => {
      ...
      let inferredInlineWildcard = cds.inferred(queryInlineNotation)
      let inferredInlineExplicit = cds.inferred(inlineExplicit)
      let inferredAbsolute = cds.inferred(absolutePaths)
    ...
    it('smart wildcard only works in local scope', () => {
      ...
      expect(() => cds.inferred(queryInlineNotation)).to.throw(/Duplicate definition of element “office_address”/)
    })
    ...
    it('wildcard - sql style on table alias', () => {
      ...
      let inferredWithoutBrackets = cds.inferred(queryInlineNotation)
      let inferredWithBrackets = cds.inferred(queryInlineNotationWithoutBrackets)
      let inferredWildcard = cds.inferred(regularWildcard)
      let { EmployeeNoUnmanaged } = model.entities
    ...
    it('wildcard - sql style on table alias with excluding', () => {
      ...
      let inferredShortcut = cds.inferred(queryInlineNotation)
      let inferredAbsolute = cds.inferred(regularWildcard)
      let { EmployeeNoUnmanaged } = model.entities
