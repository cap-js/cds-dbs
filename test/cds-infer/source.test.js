...
describe('simple', () => {
  ...
  it('infer source for UPDATE, DELETE and INSERT', () => {
    ...
    expect(cds.inferred(u)).to.have.property('target').that.equals(Authors)
    expect(cds.inferred(i)).to.have.property('target').that.equals(Books)
    expect(cds.inferred(d)).to.have.property('target').that.equals(Books)
  })
