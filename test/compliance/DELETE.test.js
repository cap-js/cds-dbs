...
describe('DELETE', () => {
  ...
  describe('from', () => {
    ...
    test('ref', async () => {
      ...
      const res = await cds.run(DELETE.from(Authors))
      expect(res.affected).to.be.eq(10, 'Ensure that all rows are affected') // 1 from csv, 9 newly added
    })
  })
  ...
  test('affected rows', async () => {
    const res = await DELETE.from('complex.associations.Books').where('ID = 4712')
