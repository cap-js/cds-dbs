    ...
    function inferQueryElement(column, insertIntoQueryElements = true, $baseLink = null, inExists = false) {
      ...
      column.ref.forEach((step, i) => {
        ...
        if (i === 0) {
          ...
          else if (inferred.outerQueries?.find(outer => id in outer.sources)) { // outer query accessed via alias
          ...
          else if(skipAliasedFkSegmentsOfNameStack[0] === id)
            skipAliasedFkSegmentsOfNameStack.shift()
          else
            nameSegments.push(id)
        }
          ...
          if (column.cast) {
            ...
            if (insertIntoQueryElements) {
              ...
              if(column.as) {
                ...
                const refNavigation = column.ref.slice(firstStepIsSelf || firstStepIsTableAlias ? 1 : 0).map(idOnly).join('_')
                if(refNavigation !== flatName)
                  elementName = refNavigation
                else
                  elementName = flatName
              }
