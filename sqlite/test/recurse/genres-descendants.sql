WITH RECURSIVE
  ...
  Descendants(HIERARCHY_LEVEL,HIERARCHY_PARENT_RANK,HIERARCHY_RANK,HIERARCHY_ROOT_RANK,HIERARCHY_TREE_SIZE,HIERARCHY_DISTANCE,NODE_ID,PARENT_ID,ID,name) AS (
    ...
    SELECT 
      ...
      Descendants.HIERARCHY_DISTANCE + 1,
      Hierarchy.NODE_ID,
      Hierarchy.PARENT_ID,
      Hierarchy.ID,
      Hierarchy.name
    FROM Descendants AS Descendants
    JOIN Hierarchy AS Hierarchy ON Hierarchy.PARENT_ID=Descendants.NODE_ID
    ORDER BY 1
  )
SELECT 
  Hierarchy.HIERARCHY_LEVEL,
  ...
  Hierarchy.HIERARCHY_ROOT_RANK,
  Hierarchy.HIERARCHY_TREE_SIZE,
  Descendants.HIERARCHY_DISTANCE,
  Hierarchy.NODE_ID,
  Hierarchy.PARENT_ID,
  Hierarchy.ID,
  Hierarchy.name
...
ON Hierarchy.HIERARCHY_RANK = Descendants.HIERARCHY_RANK