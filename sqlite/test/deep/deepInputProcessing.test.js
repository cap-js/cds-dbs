...
const { POST, PUT, expect } = cds.test(__dirname, 'deep.cds')
...
describe('UUID Generation', () => {
  ...
  test('generate UUID on update', async () => {
    ...
    expect(resUpdate.data.toOneChild.ID).to.equal(resUpdate.data.toOneChild_ID)
    expect(resUpdate.data.toOneChild.ID).to.equal(resUpdate.data.toOneChild.toManySubChild[0].backlink_ID)
    expect(resUpdate.data.toOneChild.ID).to.equal(resUpdate.data.toOneChild.toManySubChild[1].backlink_ID)
  })
