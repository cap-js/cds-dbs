...
describe('cds.stream', () => {
  beforeAll(async () => {
    ...
    await cds.run("INSERT INTO test_Images values(?,?)", [[1, data], [2, null]])
  }) 
  ...
  test('READ stream property with .from and .where', done => {
    ...
    cds.stream('data').from(Images).where({ID:1}).then( stream => 
        stream.pipe (
  ...
  test('READ stream property with object in .from', done => {
    ...
    cds.stream('data').from(Images, {ID:1}).then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('READ stream property with key in .from', done => {
    ...
    cds.stream('data').from(Images, 1).then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('READ stream property from entry that does not exist', async () => {
    const {Images} = cds.entities('test')
    try { await cds.stream('data').from(Images, 23) }
    catch(e) { expect(e.message).toEqual('Entity "test.Images" with entered keys is not found') }
  })
  ...
  test('READ stream property with key and column in .from', done => {
    ...
    cds.stream().from(Images, 1, 'data').then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('READ stream property with column as function in .from', done => {
    ...
    cds.stream().from(Images, 1, a => a.data).then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
...
const readStream = (id, done) => {
  const {Images} = cds.entities('test')
  const file = path.join (__dirname,'samples/out.jpg')
  STREAM.from(Images, {ID:id}).column('data').then( stream => 
    stream.pipe (
        fs.createWriteStream (file) .once ('finish', ()=>{
          expect(fs.statSync(file).size).toEqual(7891)
          fs.unlink (file, done)
...
describe('new STREAM API', () => {
  beforeAll(async () => {
    const data = fs.readFileSync(path.join(__dirname, 'samples/test.jpg'))
    await cds.run("INSERT INTO test_Images values(?,?)", [[1, data], [2, null], [3, data]])
  }) 
  ...
  test('READ stream property with .from, .column and .where', done => {
    ...
    STREAM.from(Images).column('data').where({ID:1}).then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('READ stream property with key as object in .from', done => {
    ...
    STREAM.from(Images, {ID:1}).column('data').then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('READ stream property with key as value in .from', done => {
    ...
    STREAM.from(Images,1).column('data').then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('READ stream property with column in STREAM', done => {
    ...
    STREAM('data').from(Images,1).then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('READ stream property from not existing entry', async () => {
    const {Images} = cds.entities('test')
    try { await STREAM('data').from(Images,15) }
    catch(e) { expect(e.message).toEqual('Entity "test.Images" with entered keys is not found') }
  })
  ...
  test('READ stream property without .where', done => {
    ...
    STREAM('data').from(Images).then( stream => 
        stream.pipe (
            fs.createWriteStream (file) .once ('finish', ()=>{
              expect(fs.statSync(file).size).toEqual(7891)
              fs.unlink (file, done)
  ...
  test('WRITE stream property with .column and .where', done => {
    ...
    STREAM.into(Images).column('data').data(stream).where({ID:1}).then( async (rowNum) => {
  ...
  test('WRITE stream property with keys as object in .into', done => {
    ...
    STREAM.into(Images,{ID:1}).column('data').data(stream).then( async (rowNum) => {
      expect(rowNum).toEqual(1)
      readStream(1,done)
    }
    )
  ...
  test('WRITE stream property with keys as integer in .into', done => {
    ...
    STREAM.into(Images,1).column('data').data(stream).then( async (rowNum) => {
      expect(rowNum).toEqual(1)
      readStream(1,done)
    }
    )
  ...
  xtest('WRITE stream property with keys and column in .into', done => {
    ...
    STREAM.into(Images,1,'data').data(stream).then( async (rowNum) => {
      expect(rowNum).toEqual(1)
      readStream(1,done)
    }
    )
  ...
  xtest('WRITE stream property with data in STREAM', done => {
    ...
    STREAM(stream).into(Images,1,'data').then( async (rowNum) => {
      expect(rowNum).toEqual(1)
      readStream(1,done)
    }
    )
