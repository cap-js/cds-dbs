...
describe('vector functions - SQL variants', () => {
  const { expect } = cds.test(__dirname + '/../../test/compliance/resources')

  describe('COSINE_SIMILARITY_SQL', () => {
    test('identical vectors return 1', async () => {
      const res = await SELECT.from('complex.associations.Books')
        .columns`cosine_similarity_sql(cast('[1, 0, 0]' as cds.Vector), cast('[1, 0, 0]' as cds.Vector)) as similarity`
      expect(res[0].similarity).to.eq(1)
    })
    test('orthogonal vectors return 0', async () => {
      const res = await SELECT.from('complex.associations.Books')
        .columns`cosine_similarity_sql(cast('[1, 0, 0]' as cds.Vector), cast('[0, 1, 0]' as cds.Vector)) as similarity`
  ...
  describe('L2DISTANCE_SQL', () => {
    test('identical vectors return 0', async () => {
      const res = await SELECT.from('complex.associations.Books')
        .columns`l2distance_sql(cast('[1, 0, 0]' as cds.Vector), cast('[1, 0, 0]' as cds.Vector)) as distance`
      expect(res[0].distance).to.eq(0)
    })
    test('unit vectors distance', async () => {
      const res = await SELECT.from('complex.associations.Books')
        .columns`l2distance_sql(cast('[1, 0, 0]' as cds.Vector), cast('[0, 1, 0]' as cds.Vector)) as distance`
      expect(Math.abs(res[0].distance - Math.SQRT2) < 0.0001).to.eq(true)
    })
    test('known distance', async () => {
      const res = await SELECT.from('complex.associations.Books')
        .columns`l2distance_sql(cast('[0, 0, 0]' as cds.Vector), cast('[3, 4, 0]' as cds.Vector)) as distance`
      expect(res[0].distance).to.eq(5)
    })
  })

  describe('L2NORMALIZE_SQL', () => {
    test('normalizes to unit length', async () => {
      const res = await SELECT.from('complex.associations.Books')
        .columns`l2normalize_sql(cast('[3, 4, 0]' as cds.Vector)) as normalized`
    ...
    test('already normalized unchanged', async () => {
      const res = await SELECT.from('complex.associations.Books')
        .columns`l2normalize_sql(cast('[1, 0, 0]' as cds.Vector)) as normalized`
      const normalized = JSON.parse(res[0].normalized)
