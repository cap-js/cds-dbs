...
describe('inline', () => {
  ...
  it('simple structural inline expansion', () => {
    let inlineQuery = cds.ql`select from nestedProjections.Employee as Employee {
      office.{
        floor,
        room
      }
    }`
    let longVersion = cds.ql`select from nestedProjections.Employee as Employee {
      office.floor,
      office.room
    }`
  ...
  it('structural inline expansion with path expression and infix filter', () => {
    let inlineQuery = cds.ql`select from nestedProjections.Department as Department {
      head[job = 'boss'].office.{
        floor
      }
    }`
    let expected = cds.ql`select from nestedProjections.Department as Department
  ...
  it('mixed with expand', () => {
    let queryInlineNotation = cds.ql`select from nestedProjections.Employee as Employee {
          office {
            floor,
            address.{
              city,
    ...
    let variantWithoutInline = cds.ql`select from nestedProjections.Employee as Employee {
      office {
        floor,
        address.city,
        address.street
  ...
  it('wildcard inline toplevel', () => {
    let inlineWildcard = cds.ql`select from nestedProjections.EmployeeNoUnmanaged as EmployeeNoUnmanaged {
      office.{ * }
    }`
    let inlineExplicit = cds.ql`select from nestedProjections.EmployeeNoUnmanaged as EmployeeNoUnmanaged {
      office.{
        floor,
        room,
        building,
  ...
  it('wildcard inline deep w/o brackets', () => {
    let inline = cds.ql`select from nestedProjections.EmployeeNoUnmanaged as EmployeeNoUnmanaged {
      office.{ address.* }
    }`
    let absolutePaths = cds.ql`select from nestedProjections.EmployeeNoUnmanaged as EmployeeNoUnmanaged {
      office.address.city,
  ...
  it('wildcard expansion sql style on table alias', () => {
    let inline = cds.ql`select from nestedProjections.EmployeeNoUnmanaged as E {
      E.*
    }`
    let inlineWithBrackets = cds.ql`select from nestedProjections.EmployeeNoUnmanaged as E {
      E.{*}
    }`
