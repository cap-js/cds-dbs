...
describe('infix filter on entities', () => {
  ...
  it('handles simple infix filter at entity', () => {
    let query = cqn4sql(cds.ql`SELECT from bookshop.Books[price < 12.13] as Books {ID}`, model)
  ...
  it('handles multiple simple infix filters at entity', () => {
    let query = cqn4sql(cds.ql`SELECT from bookshop.Books[price < 12.13 or 12.14 < price] as Books {ID}`, model)
  ...
  it('handles infix filter with struct access at entity', () => {
    let query = cqn4sql(cds.ql`SELECT from bookshop.Books[dedication.text = 'foo'] as Books {Books.ID}`, model)
  ...
  it('handles infix filter at entity with association if it accesses FK', () => {
    let query = cqn4sql(cds.ql`SELECT from bookshop.Books[author.ID = 22] as Books {Books.ID}`, model)
  ...
  it('handles query modifiers (where only) along the ref of a scoped query', () => {
    ...
    const expected = cds.ql`
      SELECT from bookshop.Authors as $a {$a.ID} where exists (
        SELECT 1 from bookshop.Books as $B WHERE $B.author_ID = $a.ID and $B.title = 'bar'
      ) GROUP BY $a.name
    `
