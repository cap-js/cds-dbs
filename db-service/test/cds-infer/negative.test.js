...
describe('negative', () => {
  ...
  describe('path traversals via $self are rejected', () => {
    it('simple field access', () => {
      const errorMessage = 'Paths starting with “$self” must not contain steps of type “cds.Association”: ref: [ $self,author,name ]'
    ...
    it('in order by', () => {
      ...
      expect(() => _inferred(CQL`SELECT from bookshop.Books{
        author
      } order by $self.author.name`, model)).to.throw(
        errorMessage,
      )
    })
    it('in group by', () => {
      ...
      expect(() => _inferred(CQL`SELECT from bookshop.Books{
        author
      } group by $self.author.name`, model)).to.throw(
        errorMessage,
      )
    })
    it('in where', () => {
      ...
      expect(() => _inferred(CQL`SELECT from bookshop.Books{
        author
      } where $self.author.name = 'King'`, model)).to.throw(
        errorMessage,
      )
    })
    it('in xpr', () => {
    ...
    it('deep field access', () => {
      const errorMessage = 'Paths starting with “$self” must not contain steps of type “cds.Association”: ref: [ $self,dedication,addressee,ID ]'
      expect(() => _inferred(CQL`SELECT from bookshop.Books{
        dedication,
    ...
    it('with inline syntax', () => {
      const errorMessage = 'Paths starting with “$self” must not contain steps of type “cds.Association”: ref: [ $self,author ]'
      expect(() => cqn4sql(CQL`SELECT from bookshop.Books{
        author,
        $self.author.{name}
      }`, model)).to.throw(
