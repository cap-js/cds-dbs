...
cds.extend (cds.ql.Query) .with (class {
  forSQL (db = cds.db || _default) { return this.flat(db.cqn4sql(this)) }
  toSQL  (db = cds.db || _default) { return _2sql(db.cqn2sql(this)) }
  toSql  (db = cds.db || _default) { return this.toSQL(db).sql }
})
