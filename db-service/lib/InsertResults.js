...
// FIXME: InsertResults is broken by json_each technique !!!
...
module.exports = class InsertResult {
  ...
  get [iterator]() {
    ...
    if (INSERT.as) return super[iterator] = function* () {
    ...
    return super[iterator] = function* () {
      for (const each of rows) yield { [k1]: this.insertedRowId4(each) }
      // REVISIT: sqlite only returns a single lastInsertRowid per row -> how is that with others?
    }
  }
