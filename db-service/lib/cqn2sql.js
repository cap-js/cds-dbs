...
const _managed = {
  '$user.id': cds.env.requires.db.impl.includes('hana') ?  'APPLICATIONUSER' : '$user.id',
  $user: cds.env.requires.db.impl.includes('hana') ? 'APPLICATIONUSER' : '$user.id',
  $now: cds.env.requires.db.impl.includes('hana') ? 'NOW' : '$now',
}
