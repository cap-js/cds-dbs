...
class PostgresService extends SQLService {
  ...
  async set(variables) {
    ...
    return Promise.all([
      (await this.prepare(`SELECT set_config(key::text,$1->>key,false) FROM json_each($1);`))
      ...
      .all([]).then((resp) => {
        ...
        if (resp.find(row => row.collname === 'en-x-icu')) this.class.CQN2SQL.prototype.orderBy = this.class.CQN2SQL.prototype.orderByICU
  ...
  static CQN2SQL = class CQN2Postgres extends SQLService.CQN2SQL {
    ...
    orderBy(orderBy,localized) {
      return orderBy.map(localized
        ? c => this.expr(c) + (c.element?.[this.class._localized] ? ` COLLATE "${this.context.locale}"` : '') + (c.sort === 'desc' || c.sort === -1 ? ' DESC' : ' ASC')
    ...
    orderByICU(orderBy,localized) {
      return orderBy.map(localized
        ? c => this.expr(c) + (c.element?.[this.class._localized] ? ` COLLATE "${this.context.locale.replace('_','-')}-x-icu"` : '') + (c.sort === 'desc' || c.sort === -1 ? ' DESC' : ' ASC')
        : c => this.expr(c) + (c.sort === 'desc' || c.sort === -1 ? ' DESC' : ' ASC')
        )
    }
  ...
  async database({ database }) {
    ...
    try {
      ...
      if(exists.rowCount && process._send) return
  ...
  async tenant({ database, tenant }) {
    ...
    try{
      await this.tx(async tx=>{
        ...
        await tx.run(`CREATE USER "${creds.user}" IN GROUP "${creds.usergroup}" PASSWORD '${creds.password}'`)
